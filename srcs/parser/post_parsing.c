...
void    input_output_checker(t_mother *s)
{
    ...
    while (cmd != NULL)
    {
        in_out = cmd->isprecededbypipe + cmd->isprecededbyche + cmd->isprecededbydoubleche;
        if (in_out > 1)
            input_corrector(s, cmd);
        in_out = cmd->isfollowedbypipe + cmd->isfollowedbyche + cmd->isfollowedbydoubleche;
        if (in_out > 1)
...
void    input_corrector(t_mother *s, t_command *cmd)
{
    ...
    if (cmd->isprecededbypipe && (cmd->isprecededbyche || cmd->isprecededbydoubleche))
    {
        temp = s->c;
        next = NULL;
        while (temp != cmd)
...
void    output_corrector(t_mother *s, t_command *cmd)
{
    ...
    if (cmd->isfollowedbypipe && (cmd->isfollowedbyche || cmd->isfollowedbydoubleche))
    {
        temp = cmd->nextpipe;
        next = NULL;
